...
var app = (function () {
    ...
    function create_fragment$6(ctx) {
    	...
    	const block = {
    		c: function create() {
    			g = svg_element("g");
    			attr_dev(g, "transform", g_transform_value = "translate(" + /*width*/ ctx[0] / 2 + " " + /*height*/ ctx[1] / 2 + ")");
    			add_location(g, file$6, 84, 0, 3312);
    		},
    		l: function claim(nodes) {
    			throw new Error("options.hydrate only works if the component was compiled with the `hydratable: true` option");
    		},
    ...
    function instance$6($$self, $$props, $$invalidate) {
    	...
    	function update(selectedIso) {
    		...
    		select(container).selectAll(".reduction-circle").data(modelReduction).join(enter => enter.append("circle").attr("class", `reduction-circle ${reduction <= 0 ? "decreased" : "increased"}`).attr("cx", 0).attr("cy", 0).attr("r", 0).call(enter => enter.transition().duration(100).attr("cx", d => d.cx).attr("cy", d => d.cy).attr("r", d => d.r)), update => update.transition().duration(100).attr("cx", d => d.cx).attr("cy", d => d.cy).attr("r", d => d.r), exit => exit.transition().duration(100).attr("cx", 0).attr("cy", 0).attr("r", 0).remove());
    	}
